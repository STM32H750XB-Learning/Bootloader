...
void W25QXX_ExitQPIMode(void)
{
    ...
    w25qxx_handle.mode = W25QXX_INTERFACE_SPI;
}
...
void W25QXX_EnterQPIMode(void)
{
    ...
    w25qxx_handle.mode = W25QXX_INTERFACE_QPI;
...
uint8_t W25QXX_ReadSR(uint8_t srx)
{
    ...
    if (w25qxx_handle.mode == W25QXX_INTERFACE_QPI)
    {
        cmd.InstructionMode = QSPI_INSTRUCTION_4_LINES;
        cmd.DataMode = QSPI_DATA_4_LINES;
    }
...
uint8_t W25QXX_UniqueID(void)
{
    ...
    if (w25qxx_handle.mode == W25QXX_INTERFACE_QPI)				/* not support for qpi mode */
    {
        return 1;
    }
    ...
    if (HAL_QSPI_Receive(&hqspi, w25qxx_handle.unique_id, 100) != HAL_OK)
    {
        return 1;
    }
